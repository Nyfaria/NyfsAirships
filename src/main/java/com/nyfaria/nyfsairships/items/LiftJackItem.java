...
public class LiftJackItem extends Item {
    ...
    public InteractionResult useOn(UseOnContext context) {
        if(context.getLevel().getBlockState(context.getClickedPos()).getBlock()!= NyfsAirships.BLOCK_CONTROL_WHEEL)
        {
    ...
    public void appendHoverText(ItemStack stack, Level world, List<Component> tooltip, TooltipFlag context) {
        ...
        if(stack.hasTag())
        {
            tooltip.add(new TranslatableComponent("liftjack.nyfsairships.tooltip"));
            tooltip.add(new TextComponent(stack.getTag().getInt("off")+" ").append(new TranslatableComponent("liftjack.nyfsairships.tooltip2")));
